...
void Lobby::textRead(){
    ...
    QString validNamePrefix = "/ValidName::";
    QString text = tcpSocket->readAll();

    if (text.startsWith(validNamePrefix)){
        QStringRef substring(&text, validNamePrefix.length(), text.length() - validNamePrefix.length());
        setLobbyPlayerList(substring.toString(), validNamePrefix);
    } else if (text.startsWith(loginPrefix)){
...
void Lobby::setLobbyPlayerList(QString list, QString delim){
    ...
    if (delim == "/ValidName::"){
        QString tempDelim = "/Name::";
        names = list.split(tempDelim, QString::SkipEmptyParts);
    } else {
        names = list.split(delim, QString::SkipEmptyParts);
    }
    ui->playerList->clear();
    ...
    if (delim == "/ValidName::"){
        ...
        this->player = ui->playerList->item(0)->text();
        ui->playerList->item(0)->setForeground(QColor(0, 0, 220));
    } else {
