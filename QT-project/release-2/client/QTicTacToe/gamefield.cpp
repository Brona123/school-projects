...
void GameField::hideEvent(QHideEvent *event){
    if (tcpSocket != NULL){
...
void GameField::changeTurn(bool clicked){
    if (myTurn == 1){
        myTurn = 2;
    } else {
        myTurn = 1;
...
void GameField::textSent(){
    tcpSocket->write("<" + player1.toLatin1() + "> " + textLine->text().toLatin1());
    textLine->clear();
}
